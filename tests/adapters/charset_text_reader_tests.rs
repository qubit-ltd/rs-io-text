...
use qubit_io::Input;
use qubit_io_text::{CharsetReadExt, CharsetTextReader, CodingErrorPolicy, TextLineRead, TextRead};
...
fn test_accessors_expose_wrapped_reader() {
    ...
    let reader = CharsetTextReader::new(input, Utf8Codec, CodingErrorPolicy::Strict);
...
fn test_with_capacity_preserves_utf8_tail_across_refills() -> std::io::Result<()> {
    ...
    let mut reader =
        CharsetTextReader::new_with_buffer_capacity(input, Utf8Codec, CodingErrorPolicy::Strict, 1);
...
fn test_charset_read_ext_reads_one_shot_limited_text() -> std::io::Result<()> {
    ...
    let output =
        input.read_to_string_with_charset_limited(Utf8Codec, CodingErrorPolicy::Strict, 3)?;
