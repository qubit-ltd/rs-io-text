...
use qubit_io::Input;
use qubit_io_text::CharsetReadExt;
...
fn test_new_decodes_utf8_text() -> std::io::Result<()> {
    ...
    let mut reader = CharsetTextReader::new(Cursor::new(bytes), Utf8Codec, CharsetDecodePolicy::report());
...
fn test_charset_text_reader_into_parts_preserves_unreturned_characters() -> std::io::Result<()> {
    let mut reader = CharsetTextReader::new(Cursor::new(b"abc".to_vec()), Utf8Codec, CharsetDecodePolicy::report());
...
fn test_new_propagates_reader_errors() {
    let mut reader = CharsetTextReader::new(FailingReader, Utf8Codec, CharsetDecodePolicy::report());
    let error = reader.read_char().expect_err("reader errors must be propagated");
...
fn test_new_rejects_invalid_bytes_in_strict_mode() {
    let mut reader = CharsetTextReader::new(Cursor::new(vec![0xFF]), Utf8Codec, CharsetDecodePolicy::report());
    let error = reader.read_char().expect_err("strict mode must reject invalid text");
...
fn test_new_reports_incomplete_bytes_in_strict_mode() {
    let mut reader = CharsetTextReader::new(Cursor::new(vec![0xE4, 0xB8]), Utf8Codec, CharsetDecodePolicy::report());
    let error = reader.read_char().expect_err("strict mode must reject incomplete text");
...
fn test_new_ignores_incomplete_bytes_in_ignore_mode() -> std::io::Result<()> {
    let mut reader = CharsetTextReader::new(Cursor::new(vec![0xE4, 0xB8]), Utf8Codec, CharsetDecodePolicy::ignore());
    let mut output = String::new();
...
fn test_with_capacity_preserves_utf8_tail_across_refills() -> std::io::Result<()> {
    ...
    let mut reader = CharsetTextReader::new_with_buffer_capacity(input, Utf8Codec, CharsetDecodePolicy::report(), 1);
...
fn test_all_small_capacities_preserve_utf8_boundaries() -> std::io::Result<()> {
    ...
    for capacity in 0..=expected.len() {
        ...
        let mut reader =
            CharsetTextReader::new_with_buffer_capacity(input, Utf8Codec, CharsetDecodePolicy::report(), capacity);
        let mut output = String::new();
...
fn test_charset_read_ext_creates_buffered_stream_reader() -> std::io::Result<()> {
    ...
    let mut reader = input.buffered_charset_text_reader(Utf8Codec, CharsetDecodePolicy::report(), 1);
    let mut output = String::new();
...
fn test_charset_read_ext_reads_one_shot_limited_text() -> std::io::Result<()> {
    ...
    let output = input.read_to_string_with_charset_limited(Utf8Codec, CharsetDecodePolicy::report(), 3)?;
